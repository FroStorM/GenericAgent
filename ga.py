...
class GenericAgentHandler(BaseHandler):
    ...
    def do_code_run(self, args, response):
        ...
        if args.get("command") and not args.get("type"):
    ...
    def do_no_tool(self, args, response):
        ...
        if re.search(r"</?(?:tool_use|tool_call|function_use)\b|<function=", content, re.I):
            yield "[Warn] Detected unparsed tool-call markup. Retrying with strict tool format.\n"
            return StepOutcome(
                {},
                next_prompt=(
                    "[System] 你上一轮输出了工具调用标签，但执行层没有成功解析。"
                    "不要结束任务，立刻重发同一个工具调用，且只能使用标准格式："
                    '<tool_use>{"name":"code_run","arguments":{"command":"..."}}</tool_use>。'
                    "不要使用 <function_use>，不要把工具调用放进普通文本或代码块。"
                )
            )
        complex_context = (
            len(" ".join(self.history_info[-6:])) > 120
            or bool(self.working.get('key_info'))
            or bool(self.working.get('related_sop'))
            or bool(re.search(r'(修改|修正|完善|SOP|文件|代码|验证|继续|处理更多|怎么做|如何做)', "\n".join(self.history_info[-4:] + [content]), re.I))
        )
        if self.current_turn < 3 and complex_context:
            yield "[Warn] Complex task attempted to finish before 3 turns; forcing deeper execution.\n"
            return StepOutcome(
                {},
                next_prompt=(
                    "[System] 当前是复杂任务，禁止在 3 轮内 no_tool 直接结束。"
                    "必须至少完成三轮：1) 读取/验证相关上下文；2) 执行修改或形成方案；3) 再次验证并给用户可执行结论。"
                    "如果上一轮输出了工具调用但格式错误，请改用标准 "
