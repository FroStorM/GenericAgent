...
class GeneraticAgent:
    ...
    def load_llm_sessions(self):
        ...
        for k, cfg in mykeys.items():
            ...
            try:
                session = None
                if 'native' in k and 'claude' in k: session = NativeToolClient(NativeClaudeSession(cfg=cfg))
                elif 'native' in k and 'oai' in k: session = NativeToolClient(NativeOAISession(cfg=cfg))
                elif 'claude' in k: session = ToolClient(ClaudeSession(cfg=cfg))
                elif 'oai' in k: session = ToolClient(LLMSession(cfg=cfg))
                elif 'mixin' in k: llm_sessions += [{'mixin_cfg': cfg}]
                if session is not None:
                    session.config_id = k
                    try: session.backend.config_id = k
    ...
    def list_llms(self): 
        self.load_llm_sessions()
        return [(i, self.get_llm_name(b), i == self.llm_no, getattr(b, 'config_id', '')) for i, b in enumerate(self.llmclients)]
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                if getattr(handler, "history_info", None):
                    self.history = handler.history_info
                self.history.append("[Agent] 上一轮执行被异常中断，任务未确认完成；后续应基于已有上下文继续恢复。")
...
if __name__ == '__main__':
    ...
    if args.bg:
        ...
        env = os.environ.copy()
        if getattr(sys, "frozen", False) or os.path.basename(sys.executable).lower() == "a3agent":
