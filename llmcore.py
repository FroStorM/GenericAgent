...
def _mykey_candidate_paths():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    roots = [
        os.environ.get('GA_USER_DATA_DIR'),
        os.environ.get('GA_WORKSPACE_ROOT'),
        os.environ.get('GA_APP_DATA_DIR'),
        base_dir,
    ]
    seen = set()
    paths = []
    for root in roots:
        ...
        candidates = [root]
        if os.path.basename(root) != os.environ.get('GA_CONFIG_DIRNAME', 'ga_config'):
            candidates.insert(0, os.path.join(root, os.environ.get('GA_CONFIG_DIRNAME', 'ga_config')))
...
def _load_mykeys():
    global _mykey_path
    for p in _mykey_candidate_paths():
        if not os.path.exists(p):
            continue
        _mykey_path = p
        if p.endswith('.py'):
            return _load_mykey_py(p)
...
def reload_mykeys():
    ...
    if mt == _mykey_mtime and 'mykeys' in globals():
        return globals()['mykeys'], False
...
_oldprint = getattr(print, "_ga_original_print", print)
def safeprint(*argv):
    try: _oldprint(*argv)
    except OSError: pass
safeprint._ga_original_print = _oldprint
print = safeprint
...
def _is_llm_error_text(text):
    text = str(text or "").strip()
    return text.startswith(("!!!Error:", "[Error:")) or "[!!! 流异常中断" in text

def _content_blocks_are_error(blocks):
    if not blocks:
        return True
    texts = [str(b.get("text", "")) for b in blocks if isinstance(b, dict) and b.get("type") == "text"]
    return bool(texts) and all(_is_llm_error_text(t) for t in texts)
...
def _openai_stream(sess, messages):
    ...
    headers = {"Authorization": f"Bearer {sess.api_key}", "Content-Type": "application/json", "Accept": "text/event-stream", "Accept-Encoding": "identity"}
    if api_mode == "responses":
...
class BaseSession:
    ...
    def ask(self, prompt):
        def _ask_gen():
            ...
            if _is_llm_error_text(content):
                with self.lock:
                    if self.history and self.history[-1] is appended_user:
                        self.history.pop()
            else:
...
class ClaudeSession(BaseSession):
    def raw_ask(self, messages):
        ...
        headers = {"x-api-key": self.api_key, "Content-Type": "application/json", "anthropic-version": "2023-06-01", "anthropic-beta": "prompt-caching-2024-07-31", "Accept-Encoding": "identity"}
...
class NativeClaudeSession(BaseSession):
    ...
    def raw_ask(self, messages):
        ...
        headers = {"Content-Type": "application/json", "anthropic-version": "2023-06-01",
            ...
            "user-agent": self.user_agent, "x-app": "cli", "Accept-Encoding": "identity"}
    ...
    def ask(self, msg):
        ...
        if _content_blocks_are_error(content_blocks):
            with self.lock:
                if self.history and self.history[-1] is appended_user:
                    self.history.pop()
        elif content_blocks:
...
"""
        ...
        if self.auto_save_tokens and self.last_tools == tools_json:
            tool_instruction = "\n### Tools: still active, **ready to call**. Use exactly `<tool_use>{\"name\":\"tool_name\",\"arguments\":{...}}</tool_use>`; do not use `<function_use>`.\n" if _en else "\n### 工具库状态：持续有效（code_run/file_read等），**可正常调用**。必须使用 `<tool_use>{\"name\":\"tool_name\",\"arguments\":{...}}</tool_use>`，不要使用 `<function_use>`。\n"
...
def _parse_text_tool_calls(content):
    ...
    def _parse_loose_parameters(text):
        args = {}
        matches = list(re.finditer(r"<parameter=([^>\s]+)>", text))
        for idx, match in enumerate(matches):
            key = match.group(1).strip()
            start = match.end()
            end = matches[idx + 1].start() if idx + 1 < len(matches) else len(text)
            raw = text[start:end]
            raw = re.sub(r"</(?:parameter|function|tool_call)>\s*$", "", raw.strip(), flags=re.DOTALL).strip()
            args[key] = _coerce_param_value(raw)
        return args
    ...
    # Some models drift to <function_use>{...}</function_use>; treat it the
    # same as <tool_use> rather than letting the agent finalize with raw tags.
    _xp = r"<(?:tool_use|tool_call|function_use)>((?:(?!<(?:tool_use|tool_call|function_use)>).){15,}?)</(?:tool_use|tool_call|function_use)>"
    ...
    if tcs:
        return tcs, content
    # try XML-ish function tags:
    # <function_use><parameter=name>file_read</parameter><parameter=arguments>{"path":"..."}</parameter></function_use>
    _fp = r"<function_use>(.*?)</function_use>"
    for s in re.findall(_fp, content, re.DOTALL):
        try:
            name_match = re.search(r"<parameter=name>(.*?)</parameter>", s, re.DOTALL)
            args_match = re.search(r"<parameter=arguments>(.*?)</parameter>", s, re.DOTALL)
            if not name_match:
                continue
            name = name_match.group(1).strip()
            args_raw = args_match.group(1).strip() if args_match else "{}"
            args = tryparse(args_raw) if args_raw else {}
            if name:
                tcs.append(MockToolCall(name, args))
        except Exception as e:
            print(f"[Warn] Failed to parse function_use: {e}")
    if tcs:
        content = re.sub(_fp, "", content, flags=re.DOTALL).strip()
        return tcs, content
    # try Claude-ish XML tool_call tags:
    # <tool_call><function=file_read><parameter=path>...</parameter><parameter=start>1</parameter></function></tool_call>
    _tcp = r"<tool_call>(.*?)</tool_call>"
    for s in re.findall(_tcp, content, re.DOTALL):
        try:
            name_match = re.search(r"<function=([^>\s]+)>", s)
            if not name_match:
                continue
            name = name_match.group(1).strip()
            args = {}
            for key, raw in re.findall(r"<parameter=([^>\s]+)>(.*?)</parameter>", s, re.DOTALL):
                args[key.strip()] = _coerce_param_value(raw)
            if not args:
                args = _parse_loose_parameters(s)
            if name:
                tcs.append(MockToolCall(name, args))
        except Exception as e:
            print(f"[Warn] Failed to parse tool_call: {e}")
    if tcs:
        content = re.sub(_tcp, "", content, flags=re.DOTALL).strip()
        return tcs, content
    # try bare malformed tool call:
    # <function=file_read> <parameter=count> 200 <parameter=path> /tmp/a.py <parameter=start> 1
    _bare = re.search(r"<function=([^>\s]+)>(.*)$", content, re.DOTALL)
    if _bare:
        name = _bare.group(1).strip()
        args = _parse_loose_parameters(_bare.group(2))
        if name and args:
            tcs.append(MockToolCall(name, args))
            content = content[:_bare.start()].strip()
    return tcs, content
